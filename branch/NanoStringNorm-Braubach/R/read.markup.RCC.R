...
read.markup.RCC <- function(rcc.path = ".", rcc.pattern = "*.RCC|*.rcc", exclude = NULL, include = NULL, nprobes = -1) {
	...
	if (!is.null(include)) {
		rcc.files <- rcc.files[rcc.files %in% include];
		}
	if (!is.null(exclude)) {
		rcc.files <- rcc.files[!rcc.files %in% exclude];
		}
	if (i <- length(intersect(include, exclude)) > 0) {
		warning(
			i,
			"\nFiles are present on both include and exclude vectors.\nFollowing files are on include list but will be ignored:\n",
			paste0(intersect(include, exclude), collapse="\n")
			);
		}
